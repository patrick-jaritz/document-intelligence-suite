...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🚀 Initializing GitHub archive with sample data...');

    // Sample repository analyses
    const sampleAnalyses = [
      {
        ...
        analysis_data: {
          ...
          technicalAnalysis: {
            ...
            unittesting: 'Comprehensive test coverage with CI/CD pipeline',
      ...
      },
      {
        ...
        analysis_data: {
          ...
          technicalAnalysis: {
            ...
            unittesting: 'Comprehensive automated testing',
      ...
      },
      {
        ...
        analysis_data: {
          ...
          technicalAnalysis: {
            ...
            unittesting: 'Comprehensive test suite with high coverage',
    ...
    if (error) {
      ...
      if (error.message.includes('relation "github_analyses" does not exist')) {
        console.log('📋 Creating github_analyses table...');
        ...
        if (retryError) {
          console.error('❌ Error inserting data after table creation:', retryError);
          return new Response(
            JSON.stringify({ 
        ...
        console.log(`✅ Successfully created table and inserted ${retryData?.length || 0} sample analyses`);
      } else {
        return new Response(
          JSON.stringify({ 
            error: 'Failed to insert sample data',
      ...
      console.log(`✅ Successfully inserted ${data?.length || 0} sample analyses`);
    }

    return new Response(
      JSON.stringify({
        success: true,
        message: 'GitHub archive initialized with sample data',
        count: sampleAnalyses.length
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200,
